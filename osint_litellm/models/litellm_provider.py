...
class LitellmProvider(models.Model):
    ...
    host = fields.Char('Host', required=False, default="http://localhost:11434")
    ...
    def action_update_providers(self):
        ...
        for provider in providers:
            existing = self.search([('name', '=', provider)], limit=1)
            if not existing:
                self.create({'name': provider.name})

        return {
            'type': 'ir.actions.act_window',
            'res_model': 'litellm.provider.list',
            'view_mode': 'list',
        }
    ...
    def action_sync_models(self):
        ...
        Capability = self.env['litellm.model.capability']
